...
import xlrd
import sys
class Ui_BattersDataset(object):
    ...
    def upload(self):
        try:
            ...
            database.commit()
            query = "insert into batters values(%s,%s,%s,%s,%s,%s,%s,%s)"
            for r in range(1, sheet.nrows):
                innings = sheet.cell(r, 0).value
                bavg = sheet.cell(r, 1).value
                sr = sheet.cell(r, 2).value
                cen = sheet.cell(r, 3).value
                fifty = sheet.cell(r, 4).value
                zero = sheet.cell(r, 5).value
                hs = sheet.cell(r, 6).value
                rating = sheet.cell(r, 7).value
                values = (str(innings), str(bavg), str(sr), str(cen), str(fifty), str(zero), str(hs), int(rating))
...
"font: 12pt \"Franklin Gothic Heavy\";")
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "Batters Dataset"))
        self.label.setText(_translate("Dialog", "Upload Batters Dataset"))
